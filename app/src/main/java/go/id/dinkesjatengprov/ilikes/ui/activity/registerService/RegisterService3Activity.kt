...
import go.id.dinkesjatengprov.ilikes.databinding.ActivityRegisterService3Binding
...
class RegisterService3Activity :
    BaseActivity<ActivityRegisterService3Binding, RegisterServiceViewModel>(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityRegisterService3Binding.inflate(layoutInflater)
    ...
    private fun initView() {
        binding?.regMactAddress?.setText(bookingModel?.patientIdentity?.addressKtp?.address)
        binding?.regMactRt?.setText(bookingModel?.patientIdentity?.addressKtp?.rt)
        binding?.regMactRw?.setText(bookingModel?.patientIdentity?.addressKtp?.rw)
        binding?.regMactProvince?.setText(bookingModel?.patientIdentity?.addressKtp?.province?.name)
        selectedProvince = bookingModel?.patientIdentity?.addressKtp?.province
        binding?.regMactCity?.setText(bookingModel?.patientIdentity?.addressKtp?.city?.name)
        selectedCity = bookingModel?.patientIdentity?.addressKtp?.city
        binding?.regMactDistrict?.setText(bookingModel?.patientIdentity?.addressKtp?.district?.name)
        selectedDistrict = bookingModel?.patientIdentity?.addressKtp?.district
        binding?.regMactSubdistrict?.setText(bookingModel?.patientIdentity?.addressKtp?.village?.name)
        selectedSubdistrict = bookingModel?.patientIdentity?.addressKtp?.village
        if (bookingModel?.patientIdentity?.addressKtp != null && bookingModel?.patientIdentity?.addressKtp == bookingModel?.patientIdentity?.addressDomicile) {
            binding?.regCbAddress?.isChecked = true
        }
    }

    override fun onClick(p0: View?) {
        if (!validation()) return
        val ktpAddress = RegionModel(
        ...
        val intent = Intent(this, RegisterService4Activity::class.java)
        intent.putExtra(BOOKING_MODEL, bookingModel)
        intent.putExtra(DOMICILE_AND_KTP, binding?.regCbAddress?.isChecked)
