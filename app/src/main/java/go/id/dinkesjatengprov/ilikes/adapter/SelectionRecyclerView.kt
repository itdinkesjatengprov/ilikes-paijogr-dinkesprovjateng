...
class SelectionRecyclerView(
...
) : RecyclerView.Adapter<SelectionRecyclerView.ViewHolder>() {

    var mSelectionText = selectionText
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        holder.binding.idsTextview.text = list?.get(position)
        if (list?.get(position) == mSelectionText) {
        ...
        holder.itemView.setOnClickListener {
            mSelectionText = list?.get(position)
